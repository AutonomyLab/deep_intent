...
def generator_model():
    ...
    model.add(TimeDistributed(Conv2D(filters=512, kernel_size=(4,4), padding='same', input_shape=(2, 4, 4, 512))))
    model.add(BatchNormalization())
    model.add(Dropout(0.5))
    ...
    # Temporal model
    x = ConvLSTM2D(filters=64, kernel_size=(4, 4), padding="same", data_format="channels_last", return_sequences=True)
    model.add(x)
    model.add(ConvLSTM2D(filters=64, kernel_size=(4, 4), padding="same", data_format="channels_last", return_sequences=True))

...
##### Future Prediction 
Our objective is to predict the future positions of salient objects like vehicles and pedestrians by learning their motion. Functionally, an encoder reads a sequence of frames ![xT](https://latex.codecogs.com/png.latex?\inline&space;\LARGE&space;\boldsymbol{x}&space;=&space;\{x_{T},...,&space;x_{1}\}) to yield dense representations ![zT](https://latex.codecogs.com/png.latex?\inline&space;\LARGE&space;\boldsymbol{z}&space;=&space;\{z_{1},...,&space;z_{T}\}). Conditioned on ![z](https://latex.codecogs.com/png.latex?\inline&space;\LARGE&space;\boldsymbol{z}), a decoder will then auto-regressively predict an image sequence ![ydashT](https://latex.codecogs.com/png.latex?\inline&space;\LARGE&space;\boldsymbol{y}^{\prime}&space;=&space;\{y_{T&plus;1}^{\prime},...,&space;y_{2T}^{\prime}\}) by minimizing a pixel-wise loss between ![ydash](https://latex.codecogs.com/png.latex?\inline&space;\LARGE&space;\boldsymbol{y}^{\prime}) and ground truth frames ![yT](https://latex.codecogs.com/png.latex?\inline&space;\LARGE&space;\boldsymbol{y}&space;=&space;\{y_{T&plus;1},...,&space;y_{2T}\}). Each generated frame is of the same resolution as the input. We reverse the temporal ordering of input data to condition the latent space with spatial information from the latest frame. The most recent frame carries forward the closest contextual resemblance. Recursively learning representations from each input frame, we expect to first learn a temporal regularity in the early representations and parametrize a temporal variance in the later representations.

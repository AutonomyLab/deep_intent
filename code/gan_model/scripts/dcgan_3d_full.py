...
def generator_model():
    ...
    # 16x32x32 image
    model.add(Conv3DTranspose(filters=64, kernel_size=(4, 4, 4), strides=(2, 2, 2), padding='same'))
    model.add(TimeDistributed(BatchNormalization()))
    model.add(TimeDistributed(LeakyReLU(0.2)))
    model.add(Dropout(0.5))

    # 32x64x64 image
    model.add(Conv3DTranspose(filters=3, kernel_size=(4, 4, 4), strides=(2, 2, 2), padding='same', activation='tanh'))
...
def train(BATCH_SIZE, GEN_WEIGHTS, DISC_WEIGHTS):
    ...
    for epoch in range(NB_EPOCHS):
        ...
        for index in range(NB_ITERATIONS):
            ...
            # while ((d_loss[len(d_loss) - 1]*100)-(g_loss[len(g_loss) - 1]*100) > 10):
            #     # Train Discriminator
            #     # image_batch = X_train[index * BATCH_SIZE:(index + 1) * BATCH_SIZE]
            #     # generated_images = generator.predict(noise, verbose=0)
            #     # X = np.concatenate((image_batch, generated_images))
        ...
        print ("\nGenerator trained: %d times Average g_loss: %f"
               "\nDiscriminator trained: %d times Average d_loss: %f"
               %(len(g_loss), avg_g_loss, len(d_loss), avg_d_loss))

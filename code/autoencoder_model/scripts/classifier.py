...
def coeff_schedule(epoch_idx):
    loss_coeff_schedule = [8, 15, 22]  # epoch_step
    print ("Changing Loss Coefficients")
    global LAMBDA
    if (epoch_idx + 1) < loss_coeff_schedule[0]:
        LAMBDA = 0
    elif (epoch_idx + 1) < loss_coeff_schedule[1]:
        LAMBDA = 0.3
    elif (epoch_idx + 1) < loss_coeff_schedule[2]:
        LAMBDA = 0.6
    else:
        LAMBDA = 1


def mse_mae_kld_loss(y_true, y_pred):
    ...
    return mse_loss + LAMBDA*mae_loss
...
def train(BATCH_SIZE, ENC_WEIGHTS, DEC_WEIGHTS, CLA_WEIGHTS):
    ...
    if CLASSIFIER:
        ...
        for epoch in range(NB_EPOCHS_CLASS):
            ...
            test_c_loss_keys = ['c_test_' + metric for metric in sclassifier.metrics_names + precs + recs]
